...
description: Run the full LLM Memory Pipeline (LMP) to collect, synthesize, cluster, and archive all Manus (and other LLM) conversation sessions. Since MPX-20260731, Git + Mem0 are the primary stores (Notion is legacy). Use when the user asks to run the full pipeline, process all sessions, do a bulk archive, or perform a periodic memory update.
status: updated
updated_by: MPX-20260731-YOS-MEMORY-GIT-MEM0-REFACTOR
---

> ⚠️ **UPDATED** — MPX-20260731-YOS-MEMORY-GIT-MEM0-REFACTOR (2026-07-31)
>
> Depuis ce MPX, **Git + Mem0 sont les stores primaires**. Notion est conservé en lecture seule (legacy).
> Le pipeline `03_archive_to_notion.py` est remplacé par `yos_memory.SessionStore`.
> Pour activer le miroir Git+Mem0 en parallèle du pipeline existant : `export YOS_MEMORY_ADAPTER=1`
> Scripts réels : `yos-automations/scripts/yos-llm-pipeline/` dans le repo YOS (pas `/home/ubuntu/manus_pipeline/`).
