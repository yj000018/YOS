...
def load_cookies(cookies_path):
    """Load cookies from Cookie-Editor JSON export format or Y-OS extractor format."""
    ...
    if isinstance(raw, list):
        cookie_dict = {}
        for c in raw:
            if 'name' not in c or not c.get('value'):
                continue
            name = c['name']
            # Handle split session-token (.0, .1) — concatenate in order
            if name.endswith('.0'):
                base = name[:-2]
                cookie_dict[base] = c['value']
            elif name.endswith('.1'):
                base = name[:-2]
                if base in cookie_dict:
                    cookie_dict[base] += c['value']
                else:
                    cookie_dict[base] = c['value']
            else:
                cookie_dict[name] = c['value']
        return cookie_dict
...
def get_access_token(cookies_dict):
    """Get ChatGPT access token via /api/auth/session using cookies."""
    ...
    try:
        with urllib.request.urlopen(req, timeout=15) as r:
            ...
            if not token:
                print(f"  [WARN] No accessToken in session response. Keys: {list(data.keys())}")
                return None
            user = data.get('user', {}).get('email', 'unknown')
            print(f"  [OK] Authenticated as: {user}")
            return token
    except urllib.error.HTTPError as e:
        print(f"  [WARN] Session endpoint HTTP {e.code}: {e.read(100).decode('utf-8', errors='replace')}")
        return None
    except Exception as e:
        print(f"  [WARN] Session endpoint error: {e}")
        return None

# ── ChatGPT API calls ──────────────────────────────────────────────────────────
def get_conversations(cookies_dict, offset=0, limit=PAGE_SIZE, bearer_token=None):
    ...
    try:
        ...
        if e.code == 401:
            print("ERROR 401: Session expired. Re-run cookie extraction.")
            sys.exit(1)
        elif e.code == 429:
            print("Rate limited. Waiting 10 seconds...")
            time.sleep(10)
            return get_conversations(cookies_dict, offset, limit, bearer_token)
        else:
            raise

def fetch_all_conversations(cookies_dict, known_ids=None, max_sessions=None, bearer_token=None):
    """
    ...
    while True:
        ...
        data = get_conversations(cookies_dict, offset=offset, limit=PAGE_SIZE, bearer_token=bearer_token)
        ...
        for item in items:
            ...
            def parse_ts(ts):
                ...
                if isinstance(ts, str):
                    return ts[:19].replace('T', ' ')  # ISO → "YYYY-MM-DD HH:MM:SS"
                try:
...
def main():
    ...
    # Get bearer token via /api/auth/session (required for ChatGPT Business)
    print("\nGetting access token...")
    bearer_token = get_access_token(cookies)
    if not bearer_token:
        print("  [WARN] Could not get bearer token — trying with cookies only")
    
    new_sessions = fetch_all_conversations(cookies, known_ids=known_ids, max_sessions=args.limit, bearer_token=bearer_token)
